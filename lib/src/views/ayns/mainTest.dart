...
import 'package:tester/src/views/widgets/custom_card_widget.dart'; // Import thêm nếu thiếu

// --- CÁC IMPORT CÁC MÀN HÌNH TAB KHÁC (THÊM VÀO ĐỂ NAV BAR HOẠT ĐỘNG) ---
import 'package:tester/src/views/campaign/campaign_list_screen.dart'; // Tab 1: Xu hướng
import 'package:tester/src/views/video/video_needy_person_screen.dart'; // Tab 2: Reels (Lấy từ file mẫu)
...
class _mainTestState extends State<mainTest> {
  ...
  // Danh sách các màn hình cho IndexedStack
  ...
  // HÀM XỬ LÝ KHI BẤM TAB (SỬA LẠI LOGIC)
  void _onTabTapped(int index) {
    if (index == 2) {
      // NẾU LÀ REELS (Tab giữa): Push màn hình mới
      // Lưu ý: Kiểm tra lại tên class VideoNeedyReelsScreen hoặc VideoNeedyPersonScreen trong project của bạn
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => const VideoNeedyReelsScreen()),
      );
    } else {
      // CÁC TAB KHÁC: Chuyển index
      setState(() {
        _currentIndex = index;
      });
    }
  ...
  // --- GIAO DIỆN TRANG CHỦ CŨ CỦA BẠN (Đã tách ra hàm riêng để dùng trong IndexedStack) ---
                  ...
                  IntrinsicHeight(
                    child: Container(
                      ...
                      decoration: const BoxDecoration(
                        ...
                        borderRadius: BorderRadius.only(
                          topLeft:
                              Radius.circular(10), // Bo tròn góc trên bên trái
                          bottomLeft:
                              Radius.circular(10), // Bo tròn góc dưới bên trái
                        ),
                      ),
                      child: const Row(
                        crossAxisAlignment: CrossAxisAlignment.center,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // SỬ DỤNG IndexedStack ĐỂ GIỮ TRẠNG THÁI VÀ CHUYỂN TAB
      body: SafeArea(
        child: IndexedStack(
          index: _currentIndex,
          children: _pages,
      ...
      bottomNavigationBar: MyNavigationBar(
        ...
        onTap: _onTabTapped, // Sử dụng hàm đã sửa logic
      ),
    );
  }
}
